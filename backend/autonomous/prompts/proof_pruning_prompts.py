...
def _snapshot_payload(snapshot: ProofPruneSnapshot, *, include_lean: bool) -> str:
    ...
    if not include_lean:
        for key in ("candidate_descriptors", "comparator_descriptors"):
...
Choose propose_prune only when the target is fully redundant or superseded by
stronger active evidence, no unique route or required dependency is lost, and
the material contribution remains represented. Do not nominate a proof merely
because it is old, long, costly, known, duplicate-novel, difficult, or unused by
the latest attempt. Missing dependency evidence is not evidence of independence.
Use only proof IDs marked eligible_candidate in the deterministic whole-set
accounting. You may choose no_prune whenever evidence is insufficient.

Return exactly this JSON object and no other fields:
{_PROPOSER_CONTRACT}

Dependent-field rules:
- no_prune requires proof_id, expected_theorem_hash, and expected_lean_hash to be null.
- propose_prune requires one non-empty proof_id and both matching canonical hashes.
...
must exactly equal {json.dumps(proposal.proof_id)}."""
