...
FIXTURE_DIR = Path(__file__).resolve().parents[1] / "fixtures" / "syntheticlib4"
...
class SyntheticLib4FixtureTests(TestCase):
    ...
    def test_syntheticlib4_records_normalize_for_search(self) -> None:
        ...
        self.assertTrue(first.canonical_identity_version)
        self.assertTrue(first.canonical_theorem_statement_hash)

    def test_moto_normalization_uses_canonical_identity_hashes(self) -> None:
        first = moto_sources.normalize_proof_record(
            ProofRecord(
                proof_id="proof_001",
                theorem_statement="theorem   spacing\n: True",
                source_type="leanoj_final",
                source_id="run_001",
                lean_code="\r\ntheorem spacing : True := by trivial\r\n",
            )
        )
        second = moto_sources.normalize_proof_record(
            ProofRecord(
                proof_id="proof_002",
                theorem_statement=" theorem spacing : True ",
                source_type="leanoj_final",
                source_id="run_002",
                lean_code="theorem spacing : True := by trivial\n",
            )
        )

        self.assertEqual(
            first.canonical_theorem_statement_hash,
            second.canonical_theorem_statement_hash,
        )
    ...
    def test_runtime_client_has_no_records_when_snapshot_is_inactive(self) -> None:
        old_data_dir = system_config.data_dir
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                system_config.data_dir = str(Path(temp_dir) / "data")
                client = SyntheticLib4Client()
            ...
            self.assertEqual(manifest["proof_count"], 0)
            self.assertEqual(records, [])
            self.assertFalse(status["membership_active"])
            self.assertEqual(status["auth_mode"], "inactive")
            self.assertFalse(validation["valid"])
            self.assertEqual(validation["proof_count"], 0)
        finally:
...
class ProofSearchIndexerTests(TestCase):
    def test_rebuild_atomically_replaces_incompatible_index(self) -> None:
        records = load_syntheticlib4_fixture_records(SyntheticLib4Client(FIXTURE_DIR))
        with tempfile.TemporaryDirectory() as temp_dir:
            index_path = Path(temp_dir) / "proof_search.sqlite"
            conn = sqlite3.connect(index_path)
            try:
                conn.execute("CREATE TABLE legacy_marker(value TEXT NOT NULL)")
                conn.execute("INSERT INTO legacy_marker(value) VALUES ('preserve-until-swap')")
                conn.execute("PRAGMA user_version = 1")
                conn.commit()
            finally:
                conn.close()

            indexer = ProofSearchIndexer(index_path)
            self.assertFalse(indexer.is_compatible())
            indexer.rebuild(records)

            self.assertTrue(indexer.is_compatible())
            conn = sqlite3.connect(index_path)
            try:
                columns = {
                    row[1]
                    for row in conn.execute("PRAGMA table_info(proof_records)").fetchall()
                }
            ...
            self.assertIn("canonical_identity_version", columns)
            self.assertIn("canonical_theorem_statement_hash", columns)
            self.assertIn("canonical_lean_code_hash", columns)
            self.assertEqual(canonical_version[0], CANONICAL_PROOF_IDENTITY_VERSION)
            self.assertFalse(
                any(index_path.parent.glob(f".{index_path.name}.rebuild-*"))
            )
    ...
    def test_exact_identity_neighborhood_scales_past_sqlite_bind_limit(self) -> None:
        base = load_syntheticlib4_fixture_records(SyntheticLib4Client(FIXTURE_DIR))[0]
        statement_hash = "shared-exact-statement"
        code_hash = "shared-exact-code"
        records = []
        expected_seed_ids = []
        ...
        for index in range(1101):
            ...
            seed_id = f"moto:seed-{index:04d}"
            neighbor_id = f"moto:neighbor-{index:04d}"
            expected_seed_ids.append(seed_id)
            expected_neighbor_ids.append(neighbor_id)
            records.extend(
                [
                    base.model_copy(
                        update={
                            **occurrence,
                            "search_id": seed_id,
                            "proof_id": f"seed-{index:04d}",
                            "external_fingerprint": f"seed-fingerprint-{index:04d}",
                            "theorem_statement_hash": statement_hash,
                            "lean_code_hash": code_hash,
                            "canonical_theorem_statement_hash": statement_hash,
                            "canonical_lean_code_hash": code_hash,
                        }
                    ),
                    base.model_copy(
                        update={
                            **occurrence,
                            "search_id": neighbor_id,
                            "proof_id": f"neighbor-{index:04d}",
                            "external_fingerprint": f"neighbor-fingerprint-{index:04d}",
                            "theorem_statement_hash": f"neighbor-statement-{index}",
                            "lean_code_hash": f"neighbor-code-{index}",
                            "canonical_theorem_statement_hash": f"neighbor-statement-{index}",
                            "canonical_lean_code_hash": f"neighbor-code-{index}",
                        }
                    ),
                ]
            )
        records.extend(
            [
                base.model_copy(
                    update={
                        "corpus": "moto",
                        "search_id": "moto:statement-only-decoy",
                        "proof_id": "statement-only-decoy",
                        "external_fingerprint": "statement-only-decoy",
                        "session_id": "decoy-session-a",
                        "run_id": "decoy-run-a",
                        "source_type": "paper",
                        "source_id": "decoy-paper-a",
                        "theorem_statement_hash": statement_hash,
                        "lean_code_hash": "wrong-code",
                        "canonical_theorem_statement_hash": statement_hash,
                        "canonical_lean_code_hash": "wrong-code",
                    }
                ),
                base.model_copy(
                    update={
                        "corpus": "moto",
                        "search_id": "moto:code-only-decoy",
                        "proof_id": "code-only-decoy",
                        "external_fingerprint": "code-only-decoy",
                        "session_id": "decoy-session-b",
                        "run_id": "decoy-run-b",
                        "source_type": "paper",
                        "source_id": "decoy-paper-b",
                        "theorem_statement_hash": "wrong-statement",
                        "lean_code_hash": code_hash,
                        "canonical_theorem_statement_hash": "wrong-statement",
                        "canonical_lean_code_hash": code_hash,
                    }
                ),
            ]
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            indexer = ProofSearchIndexer(Path(temp_dir) / "proof_search.sqlite")
            indexer.rebuild(records)
            neighborhood = indexer.exact_identity_neighborhood(
                theorem_statement_hashes=[statement_hash],
                lean_code_hashes=[code_hash],
                corpora=["moto"],
                limit=5000,
            )

        result_ids = [record.search_id for record in neighborhood]
        self.assertEqual(len(result_ids), 2202)
        self.assertEqual(result_ids[:1101], expected_seed_ids)
        self.assertEqual(result_ids[1101:], expected_neighbor_ids)
        self.assertNotIn("moto:statement-only-decoy", result_ids)
        self.assertNotIn("moto:code-only-decoy", result_ids)
    ...
    def test_moto_source_loader_keeps_duplicate_novel_and_not_novel_history(self) -> None:
        library_entries = [
            {
                "session_id": "session_a",
                "proof_id": "proof_duplicate",
                "theorem_name": "History.duplicateNovel",
                "theorem_statement": "theorem duplicate_novel : True",
                "source_type": "paper",
                "source_id": "paper_001",
                "novel": True,
            ...
            {
                "session_id": "session_a",
                "proof_id": "proof_not_novel",
                "theorem_name": "History.notNovel",
                "theorem_statement": "theorem not_novel : True",
                "source_type": "paper",
                "source_id": "paper_002",
                "novel": False,
        ...
        async def _list_library(novel_only: bool = False):
            self.assertFalse(novel_only)
            return library_entries

        async def _get_library_proof(session_id: str, proof_id: str):
            match = next(entry for entry in library_entries if entry["proof_id"] == proof_id)
            return {**match, "lean_code": f"theorem {proof_id} : True := by trivial"}

        with mock.patch.object(moto_sources.proof_database, "list_proof_library", _list_library), \
            mock.patch.object(moto_sources.proof_database, "get_library_proof", _get_library_proof):
            records = asyncio.run(moto_sources._records_from_autonomous_history())

        tiers = {record.novelty_tier for record in records}
        self.assertIn("duplicate_novel", tiers)
        self.assertIn("not_novel", tiers)
    ...
    def test_overview_and_search_routes_use_shared_service(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            service = ProofSearchService(
                index_path=Path(temp_dir) / "proof_search.sqlite",
                syntheticlib4_source=SyntheticLib4Client(FIXTURE_DIR),
            )
...
class ProofSearchFreshnessTests(TestCase):
    def test_service_rebuilds_incompatible_derived_index(self) -> None:
        record = UnifiedProofSearchRecord(
            search_id="moto::proof",
            corpus="moto",
            source_kind="verified_proof",
            proof_id="proof",
            theorem_statement="theorem proof : True",
            lean_code="theorem proof : True := by trivial",
            canonical_uri="moto-proof://proof",
        )
        with tempfile.TemporaryDirectory() as temp_dir:
            index_path = Path(temp_dir) / "proof_search.sqlite"
            indexer = ProofSearchIndexer(index_path)
            indexer.rebuild([record])
            ...
            service = ProofSearchService(index_path)
            calls = []
            ...
            service._load_records = _load_records
            asyncio.run(service.overview(include_disabled=True))
...
class SyntheticLib4RouteTests(TestCase):
    ...
    def test_status_releases_and_reindex_routes_expose_fixture_snapshot(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            fixture_client = SyntheticLib4Client(FIXTURE_DIR)
            service = ProofSearchService(
                index_path=Path(temp_dir) / "proof_search.sqlite",
                syntheticlib4_source=fixture_client,
            )
            asyncio.run(service.rebuild_index())
            app = FastAPI()
            ...
            with (
                mock.patch.object(syntheticlib4_route, "proof_search_service", service),
                mock.patch.object(syntheticlib4_route, "syntheticlib4_client", fixture_client),
            ):
                client = TestClient(app)
...
class ProofSearchToolAdapterTests(TestCase):
    ...
    def test_tactic_script_uses_assistant_without_normal_path_in_role_search(self) -> None:
        ...
        class FakeAssistantCoordinator:
            ...
            def get_latest_reusable_pack(self):
                return None
        ...
        self.assertEqual(fake_assistant.consumed, [])
        self.assertEqual(len(calls), 1)
        self.assertIsNone(calls[0].get("tools"))
        self.assertIsNone(calls[0].get("tool_choice"))

    def test_full_script_reuses_latest_assistant_pack_without_refreshing_between_attempts(self) -> None:
        agent = formalization_module.ProofFormalizationAgent(
            model_id="test-model",
            context_window=8192,
            max_output_tokens=1024,
        ...
        pack = AssistantProofPack(
            ...
            target_hash="reused_target_hash",
            query_summary="prior helpful proof context",
            results=[
                AssistantProofSupport(
                    ...
                    proof_description="A helper proof for True.",
                    lean_code="import Mathlib\n\ntheorem helper : True := by\n  trivial\n",
                    has_hydrated_code=True,
                    relevance_reason="Matches the target shape.",
                )
            ],
        )
        original_generate = formalization_module.api_client_manager.generate_completion
        ...
        class FakeAssistantCoordinator:
            ...
            def submit_target(self, snapshot):
                ...
                raise AssertionError("Lean attempts must not refresh Assistant proof memory")
        ...
        class FakeLeanResult:
            success = False
            error_output = "tactic failed"
            goal_states = "unsolved goals"

        class FakeLeanClient:
            async def check_proof(self, lean_code, timeout):
                return FakeLeanResult()
                ...
                "choices": [
                    {
                        "message": {
                            ...
                            "content": json.dumps({
                                "theorem_name": "target",
                                "lean_code": "import Mathlib\n\ntheorem target : True := by\n  trivial\n",
                                "reasoning": "Use the retrieved helper pattern.",
                            }),
                        }
                    }
                ]
        ...
        try:
            formalization_module.api_client_manager.generate_completion = fake_generate_completion
            formalization_module.get_lean4_client = lambda: FakeLeanClient()
            formalization_module.assistant_proof_search_coordinator = fake_assistant
            success, theorem_name, lean_code, attempts = asyncio.run(
                agent.prove_candidate(
                    user_research_prompt="Prove the target.",
                    source_type="paper",
                    theorem_candidate=candidate,
                    source_content="A source asks for theorem target : True.",
                    max_attempts=2,
                    source_title="Assistant full-script test",
                )
            )
        finally:
            formalization_module.api_client_manager.generate_completion = original_generate
        ...
        self.assertEqual(fake_assistant.submitted, [])
        self.assertEqual(
            [entry[0] for entry in fake_assistant.consumed],
            ["reused_target_hash", "reused_target_hash"],
        )
        self.assertEqual(len(calls), 2)
        for call in calls:
            prompt = call["messages"][0]["content"]
            self.assertIn("ASSISTANT RETRIEVED PROOF SUPPORT", prompt)
            self.assertIn("Memory.Helper", prompt)
            self.assertIn("theorem helper : True", prompt)
