...
class WolframToolLoopTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_repeated_tool_calls_after_forced_finalization_fail_specifically(
        self,
    ) -> None:
        submitter = WritingSubmitter(model_name="test-model", user_prompt="Write.")
        fake_client = FakeWolframClient()
        ...
        async def fake_generate_completion(**_kwargs):
            return {
                "choices": [
                    {
                        "message": {
                            "content": "stale assistant preamble",
                            "tool_calls": [_tool_call("again", "2+2")],
                        }
                    }
                ]
            }
        ...
        try:
            submitter_module._wolfram_tool_available = lambda: True
            wolfram_module.get_wolfram_client = lambda: fake_client
            submitter_module.api_client_manager.generate_completion = (
                fake_generate_completion
            )
            submitter_module.WOLFRAM_MAX_CALLS_PER_SUBMISSION = 1

            with self.assertRaisesRegex(
                WolframFinalizationError, "did not produce final JSON"
            ):
                await submitter._generate_completion_with_wolfram_tool(
                    task_id="task-cap",
                    initial_prompt="prompt",
                )
        finally:
