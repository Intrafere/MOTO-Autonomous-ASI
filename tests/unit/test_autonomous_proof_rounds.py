...
class _FakeResearchMetadata:
    ...
    async def get_workflow_state(self):
        return dict(self.workflow_states[-1]) if self.workflow_states else {}

    def has_interrupted_workflow(self):
        return self.interrupted_workflow

    async def get_base_user_prompt(self):
        return self.base_user_prompt

    async def set_proof_framing_state(self, **state):
        self.proof_framing_states.append(dict(state))
...
class _DeferredThenCompleteProofStage:
    ...
    async def run(self, **kwargs):
        self.calls.append(kwargs)
        candidates = kwargs.get("theorem_candidates") or [
            ProofCandidate(theorem_id="done", statement="Completed theorem."),
            ProofCandidate(theorem_id="deferred", statement="Deferred theorem."),
        ]
        if len(self.calls) == 1:
            overflow = ProofAttemptFeedback(
                attempt=1,
                theorem_id="deferred",
            ...
            await kwargs["checkpoint_callback"](
                {
                    "source_type": kwargs["source_type"],
                    "source_id": kwargs["source_id"],
                    ...
                    "status": "deferred",
                    "candidates": [
                        {"index": 1, "candidate": candidates[0].model_dump(mode="json")},
                        {"index": 2, "candidate": candidates[1].model_dump(mode="json")},
                    ],
                    "processed_candidate_ids": ["done"],
                    "deferred_candidate_ids": ["deferred"],
                    "attempts_by_candidate": {
                        "deferred": [overflow.model_dump(mode="json")],
                    },
...
class AutonomousProofFramingBoundaryTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.old_research_metadata = coordinator_module.research_metadata
        self.old_proof_database = coordinator_module.proof_database
        self.old_api_client_manager = coordinator_module.api_client_manager
    ...
    def tearDown(self):
        ...
        coordinator_module.api_client_manager = self.old_api_client_manager

    @staticmethod
    def _gate_coordinator():
        coordinator = AutonomousCoordinator()
        coordinator._allow_mathematical_proofs = True
        coordinator._base_user_research_prompt = "Prove a new combinatorial bound."
        coordinator._user_research_prompt = "effective prompt should be reset"
        coordinator._submitter_configs = [
            SimpleNamespace(
                submitter_id=1,
                model_id="first-submitter-model",
                context_window=8192,
                max_output_tokens=777,
            ),
            SimpleNamespace(
                submitter_id=2,
                model_id="second-submitter-model",
    ...
    async def test_positive_gate_persists_context_broadcasts_and_uses_first_submitter(self):
        fake_api = _FakeAPIClientManager(
            response={
                "choices": [{
                    "message": {
                        "content": (
                            '{"is_proof_amenable": true, '
                            '"reasoning": "Formal bounds directly serve the goal."}'
                        )
                    }
                }]
            }
        )
        coordinator_module.api_client_manager = fake_api
        ...
        self.assertTrue(coordinator._proof_framing_active)
        self.assertEqual(coordinator._proof_framing_context, PROOF_FRAMING_CONTEXT)
        persisted = coordinator_module.research_metadata.proof_framing_states[-1]
        self.assertTrue(persisted["active"])
        self.assertEqual(persisted["context"], PROOF_FRAMING_CONTEXT)
        self.assertEqual(
            events[-1],
            (
        ...
        self.assertEqual(call["task_id"], "proof_framing_gate_000")
        self.assertEqual(call["role_id"], "autonomous_proof_framing_gate")
        self.assertEqual(call["model"], "first-submitter-model")
        self.assertEqual(call["max_tokens"], 777)
        self.assertEqual(call["temperature"], 0.0)
        self.assertIn("Prove a new combinatorial bound.", call["messages"][0]["content"])

    async def test_negative_gate_persists_inactive_state(self):
        coordinator_module.api_client_manager = _FakeAPIClientManager(
            response={
                "choices": [{
                    "message": {
                        "content": (
                            '{"is_proof_amenable": false, '
                            '"reasoning": "The task is purely empirical."}'
                        )
                    }
                }]
            }
        ...
        self.assertEqual(persisted["reasoning"], "The task is purely empirical.")

        ordinary_prompt = build_exploration_user_prompt(
            "Develop a safer desalination control strategy.",
            [],
            [],
        )
        captured_prompt = coordinator._append_proof_framing(ordinary_prompt)
        self.assertIn("Develop a safer desalination control strategy.", captured_prompt)
        self.assertNotIn(PROOF_FRAMING_CONTEXT, captured_prompt)
        self.assertIn(
            "mathematics, theorem discovery, proof, and formalization remain first-class "
            "whenever relevant",
            captured_prompt,
        )

    async def test_fresh_start_awaits_proof_framing_gate_once_before_research(self):
        coordinator = self._gate_coordinator()
        coordinator._get_resume_point = AsyncMock(return_value=None)
        coordinator.refresh_workflow_predictions = AsyncMock()
        coordinator._broadcast_stopped_once = AsyncMock()
        coordinator._topic_exploration_phase = AsyncMock(
            side_effect=AssertionError("fresh-start audit must stop before normal research")
        )

        async def stop_after_gate():
            coordinator._stop_event.set()

        coordinator._run_proof_framing_gate = AsyncMock(side_effect=stop_after_gate)

        await coordinator.start()

        coordinator._get_resume_point.assert_awaited_once_with()
        coordinator._run_proof_framing_gate.assert_awaited_once_with()
        coordinator._topic_exploration_phase.assert_not_awaited()

    async def test_gate_model_and_parse_failures_use_inactive_fallback_with_reason(self):
        for fake_api, expected_reason in (
            (_FakeAPIClientManager(error=RuntimeError("provider unavailable")), "provider unavailable"),
            (
                _FakeAPIClientManager(
                    response={"choices": [{"message": {"content": "not valid json"}}]}
                ),
                "Proof framing gate failed:",
            ),
        ):
            with self.subTest(expected_reason=expected_reason):
                ...
                self.assertFalse(coordinator._proof_framing_active)
                self.assertEqual(coordinator._proof_framing_context, "")
                self.assertIn(expected_reason, coordinator._proof_framing_reasoning)
                persisted = coordinator_module.research_metadata.proof_framing_states[-1]
                self.assertFalse(persisted["active"])
                self.assertEqual(persisted["reasoning"], coordinator._proof_framing_reasoning)

    async def test_disabled_proof_outputs_skip_gate_model_call(self):
        fake_api = _FakeAPIClientManager()
        coordinator_module.api_client_manager = fake_api
        coordinator = self._gate_coordinator()
    ...
    def test_append_proof_framing_is_idempotent(self):
        coordinator = self._gate_coordinator()
        coordinator._proof_framing_active = True
        coordinator._proof_framing_context = PROOF_FRAMING_CONTEXT

        once = coordinator._append_proof_framing("Research goal.")
        twice = coordinator._append_proof_framing(once)

        self.assertEqual(twice, once)
        self.assertEqual(twice.count(PROOF_FRAMING_CONTEXT), 1)

    def test_append_proof_framing_defaults_legacy_instance_to_enabled(self):
        coordinator = self._gate_coordinator()
        del coordinator._allow_mathematical_proofs
        coordinator._proof_framing_active = True
        coordinator._proof_framing_context = PROOF_FRAMING_CONTEXT

        result = coordinator._append_proof_framing("Research goal.")

        self.assertEqual(result, f"Research goal.\n\n{PROOF_FRAMING_CONTEXT}")

    def test_append_proof_framing_preserves_explicit_false_suppression(self):
        coordinator = self._gate_coordinator()
        coordinator._allow_mathematical_proofs = False
        coordinator._proof_framing_active = True
        coordinator._proof_framing_context = PROOF_FRAMING_CONTEXT
        ...
        self.assertEqual(result, "Research goal.")

    def test_apply_proof_context_frames_before_library_injection(self):
        class CapturingProofDatabase:
            def __init__(self):
                self.received = None
        ...
        fake_database = CapturingProofDatabase()
        coordinator_module.proof_database = fake_database
        coordinator = self._gate_coordinator()
        coordinator._proof_framing_active = True
        coordinator._proof_framing_context = PROOF_FRAMING_CONTEXT

        result = coordinator._apply_proof_context("Research goal.")

        self.assertEqual(
            fake_database.received,
            f"Research goal.\n\n{PROOF_FRAMING_CONTEXT}",
        )
        self.assertEqual(result, f"LIBRARY::{fake_database.received}")

    async def test_resume_restores_proof_framing_without_rerunning_gate(self):
        fake_metadata = coordinator_module.research_metadata
        fake_metadata.interrupted_workflow = True
        fake_metadata.base_user_prompt = "Persisted base prompt."
        fake_metadata.workflow_states.append(
            {
                "current_tier": "tier1_aggregation",
                "proof_framing_active": True,
                "proof_framing_context": PROOF_FRAMING_CONTEXT,
                "proof_framing_reasoning": "Persisted positive decision.",
            }
        )
        fake_api = _FakeAPIClientManager(
            error=AssertionError("resume must not rerun proof framing gate")
        )
        coordinator_module.api_client_manager = fake_api
        coordinator = self._gate_coordinator()

        async def identity_normalize(state):
            return state

        coordinator._normalize_resume_state = identity_normalize
        ...
        self.assertEqual(
            coordinator._proof_framing_reasoning,
            "Persisted positive decision.",
        )
        self.assertEqual(coordinator._user_research_prompt, "Persisted base prompt.")
...
class AutonomousProofRoundTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_deferred_overflow_resumes_without_replaying_completed_candidate(self):
        fake_stage = _DeferredThenCompleteProofStage()
        coordinator = _configured_coordinator(fake_stage)
        fake_metadata = coordinator_module.research_metadata

        first_status = await coordinator._run_proof_verification(
            content="Paper source.",
            source_type="paper",
            source_id="paper_deferred",
            source_title="Paper title",
        )

        self.assertEqual(first_status, "complete")
        self.assertEqual(fake_metadata.mark_calls, [])
        self.assertEqual(fake_metadata.checkpoint["status"], "deferred")
        self.assertEqual(fake_metadata.checkpoint["processed_candidate_ids"], ["done"])

        second_status = await coordinator._run_proof_verification(
            content="Paper source.",
            source_type="paper",
            source_id="paper_deferred",
            source_title="Paper title",
        ...
        self.assertEqual(len(fake_stage.calls), 2)
        resumed = fake_stage.calls[1]["theorem_candidates"]
        self.assertEqual([candidate.theorem_id for candidate in resumed], ["deferred"])
        self.assertEqual(fake_metadata.mark_calls, ["automatic"])
